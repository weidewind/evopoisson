...
#				print " $s is $ind\n";
	...
	#			print "\nREF 2 ".ref(${$static_hash_of_nodes{$nodname}})."\n";
				push (@{$nodes_with_sub{$ind}}, \${$self ->{static_hash_of_nodes}{$nodname}}); #Ð²Ñ‹Ñ‚Ð°Ñ‰Ð¸Ñ‚ÑŒ Ð¸Ð· Ð´ÐµÑ€ÐµÐ²Ð° Ð¿Ð¾ Ð¸Ð¼ÐµÐ½Ð¸
			}
...
sub iterations_gulp_subtree_shuffling {
	...
	for (my $i = 1; $i <= $iterations; $i++){
		...
		# $rh_out_subtree->{$name}->{$site}= ìàññèâ óõîäîâ (èìåí óçëîâ)) #todo
		my %hash;
...
#	open NODECOUNT, ">$nodecount_file" or die "Cannot create $nodecount_file";
	...
	local *printer = sub {
		print " maxbin was $maxbin ";
				$maxbin = shift;
		print "now it is $maxbin\n";		
				foreach my $md(@maxdepths){ 
					foreach my $group_number(0..scalar @groups-1){
	...
	# because we iterate through site earlier $str = <GULP>;
			...
			if ($iteration_number>=50 && $iteration_number%50 == 0){
				printer($maxbin); # print to .csv files every 50 iterations, delete printed from memory
			}
		...
		#rewritten 27.02.2017
			...
			for (my $j = 0; $j < $maxbin; $j++){
				if (scalar @{$array_gbo_minus_gbe[$j]}  != scalar @{$array_cbo_minus_cbe[$j]}){
					...
					print "But it's ok, if you mix iterations and don't print NAs in .csv. I'll just compute the difference between all full arrays\n";
			...
			## medians
				...
				if (nearest(0.00000001, $group_boot_means[$i][0] - $group_boot_means[$i][1]) >= nearest(0.00000001, $obs_mean - $exp_mean)){
					$pval_env_for_mean += 1;
				}
				if (nearest(0.00000001, $group_boot_means[$i][0] - $group_boot_means[$i][1]) <= nearest(0.00000001,$obs_mean - $exp_mean)){
					$pval_epi_for_mean += 1;

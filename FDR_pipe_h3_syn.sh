...
do
		...
		do
			it=$((number+procnum*i))
				...
				( perl poissonnolim.pl -p $protein --state syn --no_neighbour_changing --simnumber 1000 --mutnum_control 0 --fake --shuffler_type exp --maxmem 10000000 --output $out ;
				perl single_sites_nolim.pl -p $protein --state syn --no_neighbour_changing --mutnum_control 0 --output $out ) &
		done
		wait
done
